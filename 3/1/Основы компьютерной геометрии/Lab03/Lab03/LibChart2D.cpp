...
CRectD::CRectD(double l, double t, double r, double b) //класс для хранения координат прямоугольника в мировых координатах
{
	left = l;
	top = t;
	right = r;
												...
												// RW -	 Область в оконных координатах - int  
{
	...
	dc.SetMapMode(MM_ANISOTROPIC); //позволяет масштабировать и трансформировать координаты
}
...
// RWX - область в окне 
{
	...
	X.RedimMatrix(nRowsX); //Изменяем размер X и Y в соответствии с количеством данных
	Y.RedimMatrix(nRowsY);
	X = XX;
	Y = YY;
	//Определяем область в мировой системе координат
