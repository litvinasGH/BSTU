...
int main(int argc, char* argv[]) {
	...
	try {
		...
		while (true) {
			...
			HT::Element* ret_element = OS13_LIB::OS13_HTCOM::Get_HT(h, storage, insertElement);

			if (!ret_element) {
				DWORD err = GetLastError();
				if (err != 0) {
					std::cerr << "Get failed. Error code: " << err << std::endl;
				}
				...
				OS13_LIB::OS13_HTCOM::Print_HT(h, ret_element);
				...
				unsigned long long ret_payload = 0;
				try {
				...
				if (!OS13_LIB::OS13_HTCOM::Update_HT(h, storage, insertElement, upd_payload, (int)strlen(upd_payload))) {
					DWORD err = GetLastError();
					if (err != 0) {
						std::cerr << "Update failed. Error code: " << err << std::endl;
					}
					if (err == ERROR_INVALID_HANDLE || storage->Addr == NULL) {
						std::cerr << "Invalid storage indicated. Closing handle and waiting for availability." << std::endl;
						OS13_LIB::OS13_HTCOM::Close_HT(h, storage);
